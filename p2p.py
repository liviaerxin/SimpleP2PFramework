...
This is a basic and simple framework on P2P (peer to peer) network, which can be applied on sharing files. 
'''
import socket 
import struct 
import threading 
...
class Peer: 
	""" Implements the core functionality that can be used by a peer in a P2P network. """ 
	...
	def __debug( self, msg ): 
		#-------------------------------------------------------------------------- 
		if self.debug: 
			""" Display messsages with the name of the current thread """
	...
	def startstabilizer( self, stabilizer, delay ): 
		#-------------------------------------------------------------------------- 
		""" Registers and starts a stabilizer function with this peer. The function will be activated every seconds. """ 
	...
	def getpeer( self, peerid ): 
		...
		# maybe make this just a return NULL? 
	...
	def sendtopeer( self, peerid, msgtype, msgdata, waitreply=True ): 
		...
		[ ( reply type, reply data ), ... ] Send a message to the identified peer. 
		""" 
		if self.router: 
	...
	def connectandsend( self, host, port, msgtype, msgdata, pid=None, waitreply=True ): 
		...
		-> [ ( reply type, reply data ), ... ] Connects and sends a message to the specified host:port. 
		""" 
		msgreply = [] 
		try: 
	...
	def checklivepeers( self ): 
		...
		try: 
			...
			self.peerlock.release() # end checklivepeers method 
		#-------------------------------------------------------------------------- 
	def mainloop( self ):
		#-------------------------------------------------------------------------- 
		...
		while not self.shutdown: 
			try: 
				...
				if self.debug: 
					traceback.print_exc() 
					continue # end while loop self.__debug( 'Main loop exiting' ) 
		s.close() # end mainloop method # end BTPeer class 
# ********************************************************** 
class PeerConnection: 
	...
	def senddata( self, msgtype, msgdata ): 
		#-------------------------------------------------------------------------- 
		""" senddata( message type, message data ) -> boolean status Send a message through 
		a peer connection. Returns True on success or False if there was an error. """ 
		try: 
	...
	def recvdata( self ): 
		#-------------------------------------------------------------------------- 
		""" recvdata() -> (msgtype, msgdata) Receive a message from a peer connection. 
		Returns (None, None) if there was any error. """ 
		try: 
			...
			if self.debug: 
				traceback.print_exc() 
			return (None, None) 
		return ( msgtype, msg ) # end recvdata method 
	#-------------------------------------------------------------------------- 
	def close( self ): 
		#-------------------------------------------------------------------------- 
